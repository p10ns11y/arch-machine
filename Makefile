.PHONY: lint validate-profiles evidence-smoke eye-comfort-test eye-comfort-generate-check ci-help archy archy-release archy-test groxy-test keeper-test
...
archy:
	cargo build --manifest-path tools/archy/Cargo.toml

archy-release:
	cargo build --release --manifest-path tools/archy/Cargo.toml
	@echo "binary: tools/archy/target/release/archy"
	@echo "run:    ./tools/archy/target/release/archy"
...
# XChat DM remote control — Rust satellite (tools/groxy, binary: groxy)
groxy-test:
