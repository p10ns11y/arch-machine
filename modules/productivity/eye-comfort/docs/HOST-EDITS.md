...
| `~/.local/lib/eye-comfort/waybar/{tn-status.sh,module.jsonc,eye-comfort.css}` | `install.sh` | Opt-in Waybar custom module (TN bar + Pango tooltip / notify). Enable in **`~/.config/waybar/config.jsonc`** (`module.jsonc`). Import **`eye-comfort.css`** into **`~/.config/waybar/style.css`** for chip gap + tooltip padding (fixes clock-year glued to tinai). Does not auto-patch Waybar config. |
